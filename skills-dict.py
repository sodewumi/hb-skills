...
def count_unique(string1):
    ...
    # I didn't use setdefault, because if the word wasn't alread in the string,
    # set deault would insert the key with the value of 0, my default. Afterward,
    # my_dict[word] would rebound to 1.Get doesn't change the list in place,
    # so it is more suited for the left hand assignment statement if the word
    # appears for the first time

    # Question1: my_dict[word] = my_dict.setdefault(word, 0) + 1
    # setdefault is a hybrid method that changes a list in place.
    # If the word appears for the first time my_dict.setdefault(word, 0) + 1
    # would evaluate to 1 and set my_dict[word] to 0. The var my_dict[word] is then rebound
    # to one. Let's say we didn't add 1 after the setdefault method so that the right hand side
    # evaluates to 0. Would my my_dict[word] actually be a reassignment because it
    # still points to the same immutable object, 0.
    for word in string1:
